...
class DBAsistenciaAppProvider with ChangeNotifier {
  List<TAsistenciaModel> listsql = []; //LISTA para alamcenar los datos

  List<TAsistenciaModel> get data => listsql;

  //Intancia del Crud de asistencai DB local.
  CrudDBAsistencia crudDb = CrudDBAsistencia();
  ...
  Future<void> insertData(TAsistenciaModel e,bool loading) async {
    isSyncing = true;
    notifyListeners();
  ...
  Future<void> updateData(TAsistenciaModel e, int? idsql, bool loading) async {
    isSyncing = true;
    notifyListeners();
...
//METODO GUARDAR EN SQLITE
  ...
  Future<void> guardarEnSQlLite( List<TAsistenciaModel> api, BuildContext context) async {
    incidenciasCarga = [];
      ...
      if (!api.any((x) => x.id == e.id)) { 
              ...
              return AlertDialog(
                ...
                content: Column(
                  ...
                  children: [
                    ...
                    H2Text(
                      text:
                          '"${e.nombrePersonal} | ${e.actividadRol}"',
                      maxLines: 40,
                      fontSize: 12,
        ...
        if (eliminar) {
          ...
          incidenciasCarga.add(
              'ELIMINADO => (ID: ${e.idsql!}): "${e.nombrePersonal}" ya no existe en el servidor y ha sido eliminado localmente.');
        } else {
      ...
      if (index != -1) {
        ...
        if (e.updated!.compareTo(listsql[index].updated!) > 0) {
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: ContentCargaDatos(
                  ...
                  comlun1: 'BDLocal',
                  column2: 'ServidorAPI',
                  text: 'El producto local "${listsql[index].nombrePersonal}" ha sido modificado en el servidor resientemente en el servidor por "${e.nombrePersonal}".' +
                      ' ¿Desea sobrescribirlo con los datos más recientes? Nuevo dato: ${e.nombrePersonal}.\n',
                ),
                actions: <Widget>[
                  TextButton(
                    onPressed: () =>
          ...
          if (overwrite) {
            ...
            incidenciasCarga.add(
                'MODIFICADO => (ID: ${listsql[index].idsql}): "${e.nombrePersonal}" ha sido actualizado debido a cambios en el servidor.');
          } else {
            incidenciasCarga.add(
                'SIN MODIFICACIÓN => (ID: ${listsql[index].idsql}): "${e.nombrePersonal}" ya existe localmente y no ha experimentado cambios.');
          }
        } else {
...
// En caso contrario, se evita la modificación, ya que la fecha de modificación más reciente es la local.
        ...
        incidenciasCarga.add(
            'CREADO NUEVO: "${e.nombrePersonal}" ha sido agregado exitosamente a la base de datos local.');
      }
    }
    offlineSaving = false;
    notifyListeners();
...
//METODO SINCRONIZACION OFFLINE
  ...
  Future<void> syncLocalDataToServer(BuildContext context) async {
    ...
    List<TAsistenciaModel> listaApi = dataProvider.listAsistencia;
    offlineSync = true;
    notifyListeners();
    ...
    for (var e in listsql) {
      ...
      if (listaApi.any((p) => p.id == e.id)) {
        ...
        if (index != -1 && index < listsql.length) {
          ...
          if (e.updated!.compareTo(listaApi[index].updated!) > 0) {
              ...
              builder: (BuildContext context) {
                return AlertDialog(
                  ...
                  content: ContentCargaDatos(
                    ...
                    column2: 'BDLocal',
                    comlun1: 'ServidorAPI',
                    text: 'El producto  "${listaApi[index].nombrePersonal}" ha sido modificado localmente a "${e.nombrePersonal}"' +
                        '  . ¿Desea sincronizar y actualizar el nuevo valor "${e.nombrePersonal}" ? Se guardarán los cambios en el servidor.',
                  ),
            ...
            if (overwrite) {
              ...
              incidenciasSinc.add(
                  'MODIFICADO : ${e.nombrePersonal} : ID ${e.id}, ha sido modificado localmente. La fecha de modificación local es más reciente que la registrada en el servidor.');
            } else {
              incidenciasSinc.add(
                  'SIN MODIFICACIÓN: ${e.nombrePersonal} no ha sufrido cambios. No se sincroniza. Desición del usuario');
            }
          } else {
            ...
            incidenciasSinc.add(
                'SIN MODIFICACIÓN: ${e.nombrePersonal} no ha sufrido cambios. No se sincroniza. Posibles casos, hay una motificación mas reciente en el servidor.');
          }
        } else {
          ...
          await dataProvider.saveProductosApp(e); //UPDATE xq no tien ID
              incidenciasSinc.add(
                'INDICE NO VALIDO ${e.nombrePersonal} : ID ${e.id} ENVIADO AL SERVIDOR, MODIFICADO=> Posible discrepancia entre el servidor y la base de datos local debido a diferencias en la cantidad de elementos.',
              );
        ...
        if (e.id == '') {
          ...
          TAsistenciaModel productoEnListaApi = listaApi.firstWhere(
            (k) =>
                // k.idEmpleados == e.idEmpleados &&
                // k.idTrabajo == e.idTrabajo &&
                k.nombrePersonal == e.nombrePersonal && 
                k.actividadRol == e.actividadRol// && 
          ...
          if (productoEnListaApi != null) {
            ...
            incidenciasSinc.add('ENVIADO AL SERVIDOR => ${e.nombrePersonal} : ID ${e.id} => PRODUCTO CREADO EN LOCAlMENTE. Enviado con exito Al servidor.');
          } else {
            // El producto no se encontró en la listaApi, maneja según tus requisitos.
            incidenciasSinc.add(
                'ERROR => ${e.nombrePersonal} : El producto no se encontró en la listaApi después de guardarlo en el servidor. Verifica la integridad de tus datos.');
          }
          ...
          await deleteData(e.idsql!, false);
          incidenciasSinc.add(
              'ELIMINADO LOCALMENTE => ${e.nombrePersonal} : ID ${e.id} => Producto eliminado en el servidor. No es necesario recrearlo. Se elimino en localmente tambien.');
        }
      }
    }
    offlineSync = false;
