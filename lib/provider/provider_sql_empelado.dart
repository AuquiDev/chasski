...
//Intancia del Crud de asistencai DB local.
  ...
  Future<void> insertData(TEmpleadoModel e) async {
    ...
    await crudDb.insertarEmpleado(e);
  ...
  Future<void> updateData(TEmpleadoModel e, int? idsql) async {
    ...
    await crudDb.updateEmpleado(e, idsql);
  ...
  Future<void> deleteData(int idsql) async {
    await crudDb.deleteEmpleado(idsql);
    await initDatabase();
    print('Delete LOcal');
    notifyListeners();
  }
...
//METODO GUARDAR EN SQLITE
  ...
  Future<void> guardarEnSQlLite(
      List<TEmpleadoModel> api, BuildContext context) async {
    incidenciasCarga = [];
