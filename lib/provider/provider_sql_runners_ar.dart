...
import 'package:chasski/offline/t_compartiva_runners_sql.dart';
...
class DBRunnersAppProvider with ChangeNotifier {
  List<TRunnersModel> listsql = []; //LISTA para alamcenar los datos

  List<TRunnersModel> get data => listsql;

  //Intancia del Crud de asistencai DB local.
  CrudDBRunners crudDb = CrudDBRunners();
  ...
  Future<void> insertData(TRunnersModel e,bool loading) async {
    isSyncing = true;
    notifyListeners();
  ...
  Future<void> updateData(TRunnersModel e, int? idsql, bool loading) async {
    isSyncing = true;
    notifyListeners();
...
//METODO GUARDAR EN SQLITE
  ...
  Future<void> guardarEnSQlLite( List<TRunnersModel> api, BuildContext context) async {
    incidenciasCarga = [];
          ...
          bool overwrite = await showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: ContentCargaDatosRunners(
                  listsql: listsql,
                  index: index,
                  e: e,
                  comlun1: 'BDLocal',
    ...
    for (var e in listsql) {
      ...
      if (listaApi.any((p) => p.id == e.id)) {
        ...
        if (e.id == '') {
          ...
          TRunnersModel productoEnListaApi = listaApi.firstWhere(
            (k) =>
