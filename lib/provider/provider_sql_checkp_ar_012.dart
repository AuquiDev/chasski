...
// import 'package:chasski/sqllite/db_crud_check_ar_p011.dart';
import 'package:chasski/sqllite/db_crud_check_ar_p012.dart';
...
class DBCheckPointsAppProviderAr012 with ChangeNotifier {
  ...
  CrudDBCheckPointsAR012 crudDb = CrudDBCheckPointsAR012();
  ...
  Future<void> insertData(TCheckPointsModel e,bool loading) async {
    isSyncing = true;
    notifyListeners();
  ...
  Future<void> updateData(TCheckPointsModel e, int? idsql, bool loading) async {
    isSyncing = true;
    notifyListeners();
...
//METODO GUARDAR EN SQLITE
  ...
  Future<void> guardarEnSQlLite( List<TCheckPointsModel> api, BuildContext context) async {
    incidenciasCarga = [];
          ...
          bool overwrite = await showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: ContentCargaDatosCheckpoints(
                  listsql: listsql,
                  index: index,
                  e: e,
                  comlun1: 'BDLocal',
...
//METODO SINCRONIZACION OFFLINE
  ...
  Future<void> syncLocalDataToServer(BuildContext context) async {
    ...
    final dataProvider =
        Provider.of<TCheckP012Provider>(context, listen: false);
    ...
    for (var e in listsql) {
      ...
      if (listaApi.any((p) => p.id == e.id)) {
        ...
        if (e.id == '') {
          ...
          TCheckPointsModel productoEnListaApi = listaApi.firstWhere(
            (k) =>
