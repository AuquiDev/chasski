...
import 'package:chasski/sqllite/crud%20check%20point/db_crud_check_ar_p01.dart';
...
class DBChPProvider01 with ChangeNotifier {
  ...
  CrudDBCheckPointsAR01 crudDb = CrudDBCheckPointsAR01();
...
// No te olvides de llamarlo en el método get
  ...
  Future<void> guardarEnSQlLite({
    ...
    for (var runnerSQL in listsql) {
      ...
      bool existInRunner =
          listServer.any((runner) => runner.id == runnerSQL.id);
        ...
        bool eliminar = await showDialog(
              ...
              builder: (BuildContext context) {
                ...
                return AssetAlertDialogPlatform(
                  ...
                  message:
                      'Este registro solo existe en tu teléfono. ¿Deseas eliminarlo y cargar los datos del servidor?',
                  child: contentDialog(
                    context: context,
                    index: index,
                    runnerSQL: runnerSQL,
    ...
    for (var runneServer in listServer) {
      ...
      final index =
          listsql.indexWhere((runnerSQL) => runneServer.id == runnerSQL.id);
          ...
          bool overwrite = await showDialog(
                ...
                builder: (BuildContext context) {
                  ...
                  return AssetAlertDialogPlatform(
                    ...
                    message:
                        'El registro ha sido modificado en el servidor. ¿Deseas actualizar la información local más reciente?',
                    child: contentDialog(
                      context: context,
                      index: index,
  ...
  Future<void> syncLocalDataToServer({
    ...
    final dataProvider = Provider.of<TCheckP01Provider>(context, listen: false);
...
// Método para procesar cada lote de datos
  Future<void> _processBatchSync(
    ...
    TCheckP01Provider dataProvider,
    List<TCheckPointsModel> listServer,
    BuildContext context,
  ) async {
    for (var runnerSQL in batch) {
      //TODOS EL error aqui es buscar id de registro ya que no, se buscara
      //y encontrara el elemnto pero po id
      //eso no basta ya que solobuscara un registro con el id que ocincia,
      //lo que se requiere es verificar si un corredor ya esta regitrado en un punto de contorl es basta
      // final index = listServer.indexWhere((runner) => runner.id == runnerSQL.id);
      //CORREGIDO: en ves de busacr po id checkpoint , buscamos por idCorredor
      //Si un corredor ya existe entonces .. .
      final index = listServer
          .indexWhere((runner) => runner.idCorredor == runnerSQL.idCorredor);
      bool existsInServer = index != -1;
      print(
          '*************************   $existsInServer   ********************');
      if (existsInServer) {
        // Si el dato existe en el servidor
        final runneServer = listServer[index]; // Obtener el dato del servidor
        bool isModifiServer = isComratedField(X: runnerSQL, Y: runneServer);
        ...
        if (isModifiServer) {
          ...
          if (overwrite) {
            //TODOS tenemso que asiganr ese ID del registro que existe para que se pueda modificar en ambos.
            ...
            TCheckPointsModel? productoEnListaApi = listServer.firstWhere(
              (k) =>
                  k.idCorredor == runnerSQL.idCorredor 
                  && k.dorsal == runnerSQL.dorsal,
            ...
            if (productoEnListaApi != null) {
              if (!toBeUpdate.contains(runnerSQL)) {
                ...
                await dataProvider
                    .saveProductosApp(runnerSQL); //Modificar en el servidor
              ...
              runnerSQL.fecha = productoEnListaApi.fecha;
                  print(
                    '*************************UPDATE   ${runnerSQL.id}   ********************');
                await updateData(runnerSQL,
                    runnerSQL.idsql); // Modificar localmente
                toBeUpdate.add(runnerSQL);
                notifyListeners();
              } else {
        ...
        // Si el dato no existe en el servidor: y si no tiene ID se guarda.
