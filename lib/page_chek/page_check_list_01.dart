...
class QrPage01ChList extends StatefulWidget {
  const QrPage01ChList(
      {super.key, required this.idCheckList, required this.name});
  final String idCheckList;
  final String name;

  @override
  State<QrPage01ChList> createState() => _QrPage01ChListState();
}

class _QrPage01ChListState extends State<QrPage01ChList> {
  ...
  Future<void> _scanQRCode() async {
    try {
      ...
      if (person.id != null) {
        ...
        final listServer = Provider.of<TCheckList01Provider>(context, listen: false).listAsistencia;
...
//SERVER
  Future<void> editarEntrada() async {
    await context.read<TCheckList01Provider>().updateTAsistenciaProvider(
          id: personScaner.id,
          idCorredor: isPerson.id!,
          idCheckList: widget.idCheckList,
          fecha: DateTime.now(),
  ...
  Future<void> guardarEntrada() async {
    await context.read<TCheckList01Provider>().postTAsistenciaProvider(
          id: '',
          idCorredor: isPerson.id!,
          idCheckList: widget.idCheckList,
          fecha: DateTime.now(),
