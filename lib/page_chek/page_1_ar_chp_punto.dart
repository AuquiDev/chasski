...
class QrPageAR01ChP extends StatefulWidget {
  const QrPageAR01ChP(
  ...
  @override
  State<QrPageAR01ChP> createState() => _QrPageAR01ChPState();
}

class _QrPageAR01ChPState extends State<QrPageAR01ChP> {
  FlutterTts flutterTts = FlutterTts();
  @override
  void initState() {
    Provider.of<DBRunnersAppProvider>(context, listen: false).initDatabase();
    Provider.of<DBCheckPointsAppProviderAr01>(context, listen: false).initDatabase();
...
class _QrScannerAR0State extends State<QrScannerAR0> {
  ...
  Future<void> _scanQRCode() async {
    try {
      ...
      if (person.id != null) {
        ...
        final listSql =   Provider.of<DBCheckPointsAppProviderAr01>(context, listen: false).listsql;
        final listServer = Provider.of<TCheckP01Provider>(context, listen: false).listAsistencia;
  ...
  Future<void> enviaroffline() async {
    ...
    await context.read<DBCheckPointsAppProviderAr01>().insertData(offlineData, true);
  }
  ...
  Future<void> editarffline() async {
    ...
    await context
        .read<DBCheckPointsAppProviderAr01>()
        .updateData(offlineData, personScaner.idsql!, true);
  }

//SERVER
  Future<void> editarEntrada() async {
    await context.read<TCheckP01Provider>().updateTAsistenciaProvider(
          id: personScaner.id,
          idCorredor: isPerson.id!,
          idCheckPoints: widget.idCheckPoints,
          fecha: DateTime.now(),
  ...
  Future<void> guardarEntrada() async {
    await context.read<TCheckP01Provider>().postTAsistenciaProvider(
          id: '',
          idCorredor: isPerson.id!,
          idCheckPoints: widget.idCheckPoints,
          fecha: DateTime.now(),
