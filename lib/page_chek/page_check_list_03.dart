...
class QrPage03ChList extends StatefulWidget {
  const QrPage03ChList(
      {super.key, required this.idCheckList, required this.name});
  final String idCheckList;
  final String name;

  @override
  State<QrPage03ChList> createState() => _QrPage03ChListState();
}

class _QrPage03ChListState extends State<QrPage03ChList> {
  ...
  Future<void> _scanQRCode() async {
    try {
      ...
      if (person.id != null) {
        ...
        final listServer = Provider.of<TCheckList03Provider>(context, listen: false).listAsistencia;
...
//SERVER
  Future<void> editarEntrada() async {
    await context.read<TCheckList03Provider>().updateTAsistenciaProvider(
          id: personScaner.id,
          idCorredor: isPerson.id!,
          idCheckList: widget.idCheckList,
          fecha: DateTime.now(),
  ...
  Future<void> guardarEntrada() async {
    await context.read<TCheckList03Provider>().postTAsistenciaProvider(
          id: '',
          idCorredor: isPerson.id!,
          idCheckList: widget.idCheckList,
          fecha: DateTime.now(),
