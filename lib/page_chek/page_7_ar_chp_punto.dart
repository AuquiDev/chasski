...
class QrPageAR07ChP extends StatefulWidget {
  const QrPageAR07ChP(
  ...
  @override
  State<QrPageAR07ChP> createState() => _QrPageAR07ChPState();
}

class _QrPageAR07ChPState extends State<QrPageAR07ChP> {
  FlutterTts flutterTts = FlutterTts();
  @override
  void initState() {
    Provider.of<DBRunnersAppProvider>(context, listen: false).initDatabase();
    Provider.of<DBCheckPointsAppProviderAr07>(context, listen: false).initDatabase();
...
class _QrScannerAR0State extends State<QrScannerAR0> {
  ...
  Future<void> _scanQRCode() async {
    try {
      ...
      if (person.id != null) {
        ...
        final listSql =   Provider.of<DBCheckPointsAppProviderAr07>(context, listen: false).listsql;
        final listServer = Provider.of<TCheckP07Provider>(context, listen: false).listAsistencia;
  ...
  Future<void> enviaroffline() async {
    ...
    await context.read<DBCheckPointsAppProviderAr07>().insertData(offlineData, true);
  }
  ...
  Future<void> editarffline() async {
    ...
    await context
        .read<DBCheckPointsAppProviderAr07>()
        .updateData(offlineData, personScaner.idsql!, true);
  }

//SERVER
  Future<void> editarEntrada() async {
    await context.read<TCheckP07Provider>().updateTAsistenciaProvider(
          id: personScaner.id,
          idCorredor: isPerson.id!,
          idCheckPoints: widget.idCheckPoints,
          fecha: DateTime.now(),
  ...
  Future<void> guardarEntrada() async {
    await context.read<TCheckP07Provider>().postTAsistenciaProvider(
          id: '',
          idCorredor: isPerson.id!,
          idCheckPoints: widget.idCheckPoints,
          fecha: DateTime.now(),
