...
class QrPageAR08ChP extends StatefulWidget {
  const QrPageAR08ChP(
  ...
  @override
  State<QrPageAR08ChP> createState() => _QrPageAR08ChPState();
}

class _QrPageAR08ChPState extends State<QrPageAR08ChP> {
  FlutterTts flutterTts = FlutterTts();
  @override
  void initState() {
    Provider.of<DBRunnersAppProvider>(context, listen: false).initDatabase();
    Provider.of<DBCheckPointsAppProviderAr08>(context, listen: false).initDatabase();
...
class _QrScannerAR0State extends State<QrScannerAR0> {
  ...
  Future<void> _scanQRCode() async {
    try {
      ...
      if (person.id != null) {
        ...
        final listSql =   Provider.of<DBCheckPointsAppProviderAr08>(context, listen: false).listsql;
        final listServer = Provider.of<TCheckP08Provider>(context, listen: false).listAsistencia;
  ...
  Future<void> enviaroffline() async {
    ...
    await context.read<DBCheckPointsAppProviderAr08>().insertData(offlineData, true);
  }
  ...
  Future<void> editarffline() async {
    ...
    await context
        .read<DBCheckPointsAppProviderAr08>()
        .updateData(offlineData, personScaner.idsql!, true);
  }

//SERVER
  Future<void> editarEntrada() async {
    await context.read<TCheckP08Provider>().updateTAsistenciaProvider(
          id: personScaner.id,
          idCorredor: isPerson.id!,
          idCheckPoints: widget.idCheckPoints,
          fecha: DateTime.now(),
  ...
  Future<void> guardarEntrada() async {
    await context.read<TCheckP08Provider>().postTAsistenciaProvider(
          id: '',
          idCorredor: isPerson.id!,
          idCheckPoints: widget.idCheckPoints,
          fecha: DateTime.now(),
