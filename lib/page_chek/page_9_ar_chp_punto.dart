...
class QrPageAR09ChP extends StatefulWidget {
  const QrPageAR09ChP(
  ...
  @override
  State<QrPageAR09ChP> createState() => _QrPageAR09ChPState();
}

class _QrPageAR09ChPState extends State<QrPageAR09ChP> {
  FlutterTts flutterTts = FlutterTts();
  @override
  void initState() {
    Provider.of<DBRunnersAppProvider>(context, listen: false).initDatabase();
    Provider.of<DBCheckPointsAppProviderAr09>(context, listen: false).initDatabase();
...
class _QrScannerAR0State extends State<QrScannerAR0> {
  ...
  Future<void> _scanQRCode() async {
    try {
      ...
      if (person.id != null) {
        ...
        final listSql =   Provider.of<DBCheckPointsAppProviderAr09>(context, listen: false).listsql;
        final listServer = Provider.of<TCheckP09Provider>(context, listen: false).listAsistencia;
  ...
  Future<void> enviaroffline() async {
    ...
    await context.read<DBCheckPointsAppProviderAr09>().insertData(offlineData, true);
  }
  ...
  Future<void> editarffline() async {
    ...
    await context
        .read<DBCheckPointsAppProviderAr09>()
        .updateData(offlineData, personScaner.idsql!, true);
  }

//SERVER
  Future<void> editarEntrada() async {
    await context.read<TCheckP09Provider>().updateTAsistenciaProvider(
          id: personScaner.id,
          idCorredor: isPerson.id!,
          idCheckPoints: widget.idCheckPoints,
          fecha: DateTime.now(),
  ...
  Future<void> guardarEntrada() async {
    await context.read<TCheckP09Provider>().postTAsistenciaProvider(
          id: '',
          idCorredor: isPerson.id!,
          idCheckPoints: widget.idCheckPoints,
          fecha: DateTime.now(),
