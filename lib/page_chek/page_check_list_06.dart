...
class QrPage06ChList extends StatefulWidget {
  const QrPage06ChList(
      {super.key, required this.idCheckList, required this.name});
  final String idCheckList;
  final String name;

  @override
  State<QrPage06ChList> createState() => _QrPage06ChListState();
}

class _QrPage06ChListState extends State<QrPage06ChList> {
  ...
  Future<void> _scanQRCode() async {
    try {
      ...
      if (person.id != null) {
        ...
        final listServer = Provider.of<TCheckList06Provider>(context, listen: false).listAsistencia;
...
//SERVER
  Future<void> editarEntrada() async {
    await context.read<TCheckList06Provider>().updateTAsistenciaProvider(
          id: personScaner.id,
          idCorredor: isPerson.id!,
          idCheckList: widget.idCheckList,
          fecha: DateTime.now(),
  ...
  Future<void> guardarEntrada() async {
    await context.read<TCheckList06Provider>().postTAsistenciaProvider(
          id: '',
          idCorredor: isPerson.id!,
          idCheckList: widget.idCheckList,
          fecha: DateTime.now(),
