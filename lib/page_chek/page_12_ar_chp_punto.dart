...
class QrPageAR012ChP extends StatefulWidget {
  const QrPageAR012ChP(
      {super.key, required this.idCheckPoints, required this.name});
  final String idCheckPoints;
  final String name;

  @override
  State<QrPageAR012ChP> createState() => _QrPageAR012ChPState();
}

class _QrPageAR012ChPState extends State<QrPageAR012ChP> {
  FlutterTts flutterTts = FlutterTts();
  @override
  void initState() {
    Provider.of<DBRunnersAppProvider>(context, listen: false).initDatabase();
    Provider.of<DBCheckPointsAppProviderAr012>(context, listen: false).initDatabase();
...
class _QrScannerAR0State extends State<QrScannerAR0> {
  ...
  Future<void> _scanQRCode() async {
    try {
      ...
      if (person.id != null) {
        ...
        final listSql =   Provider.of<DBCheckPointsAppProviderAr012>(context, listen: false).listsql;
        final listServer = Provider.of<TCheckP012Provider>(context, listen: false).listAsistencia;
  ...
  Future<void> enviaroffline() async {
    ...
    await context.read<DBCheckPointsAppProviderAr012>().insertData(offlineData, true);
  }
  ...
  Future<void> editarffline() async {
    ...
    await context
        .read<DBCheckPointsAppProviderAr012>()
        .updateData(offlineData, personScaner.idsql!, true);
  }

//SERVER
  Future<void> editarEntrada() async {
    await context.read<TCheckP012Provider>().updateTAsistenciaProvider(
          id: personScaner.id,
          idCorredor: isPerson.id!,
          idCheckPoints: widget.idCheckPoints,
          fecha: DateTime.now(),
  ...
  Future<void> guardarEntrada() async {
    await context.read<TCheckP012Provider>().postTAsistenciaProvider(
          id: '',
          idCorredor: isPerson.id!,
          idCheckPoints: widget.idCheckPoints,
          fecha: DateTime.now(),
