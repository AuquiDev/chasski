...
class QrPage07ChList extends StatefulWidget {
  const QrPage07ChList(
      {super.key, required this.idCheckList, required this.name});
  final String idCheckList;
  final String name;

  @override
  State<QrPage07ChList> createState() => _QrPage07ChListState();
}

class _QrPage07ChListState extends State<QrPage07ChList> {
  ...
  Future<void> _scanQRCode() async {
    try {
      ...
      if (person.id != null) {
        ...
        final listServer = Provider.of<TCheckList07Provider>(context, listen: false).listAsistencia;
...
//SERVER
  Future<void> editarEntrada() async {
    await context.read<TCheckList07Provider>().updateTAsistenciaProvider(
          id: personScaner.id,
          idCorredor: isPerson.id!,
          idCheckList: widget.idCheckList,
          fecha: DateTime.now(),
  ...
  Future<void> guardarEntrada() async {
    await context.read<TCheckList07Provider>().postTAsistenciaProvider(
          id: '',
          idCorredor: isPerson.id!,
          idCheckList: widget.idCheckList,
          fecha: DateTime.now(),
