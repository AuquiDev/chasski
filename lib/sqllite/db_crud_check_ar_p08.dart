...
class CrudDBCheckPointsAR08 {
  ...
  Future<List<TCheckPointsModel>> getTable() async {
    ...
    List<Map<String, dynamic>> respponse = await db.query('check_points08');
  ...
  Future<void> deleteTable(int idsql) async {
    Database db = await _databaseTablesDB.checkinDatabase();
    await db.delete('check_points08', where: 'idsql = ?', whereArgs: [idsql]);
  }

  //Borrar  todos los datos
  Future<void> clearDataBase() async {
    Database db = await _databaseTablesDB.checkinDatabase();
    await db.delete('check_points08');
