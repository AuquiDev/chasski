...
class TChecklist_06 {
  ...
  static getAsitenciaPk() async {
    final records = await pb.collection('ar_chl_6_entrega_medallas').getFullList(
          sort: '-created',
        );
    // print(records);
    return records;
  ...
  static  postAsistenciaPk(TChekListmodel01 data) async {
    final record =
        await pb.collection('ar_chl_6_entrega_medallas').create(body: data.toJson());
    return record;
  }

  static  putAsitneciaPk({String? id, TChekListmodel01? data}) async {
    final record =
        await pb.collection('ar_chl_6_entrega_medallas').update(id!, body: data!.toJson());
    return record;
  }

  static Future  deleteAsistentciaPk(String id) async {
    final record = await pb.collection('ar_chl_6_entrega_medallas').delete(id);
    return record;
  }
