...
class TCheckPoitns_AR013 {
  static getAsitenciaPk() async {
    final records = await pb.collection('ar_chpr_13_punto').getFullList(
          sort: '-created',
        );
    // print(records);
    return records;
  ...
  static  postAsistenciaPk(TCheckPointsModel data) async {
    final record =
        await pb.collection('ar_chpr_13_punto').create(body: data.toJson());
  ...
  static  putAsitneciaPk({String? id, TCheckPointsModel? data}) async {
    final record =
        await pb.collection('ar_chpr_13_punto').update(id!, body: data!.toJson());
    return record;
  }

  static Future  deleteAsistentciaPk(String id) async {
    final record = await pb.collection('ar_chpr_13_punto').delete(id);
    return record;
  }
