...
class TCheckPoitns_AR0META {
  static getAsitenciaPk() async {
    final records = await pb.collection('ar_chpr_meta').getFullList(
          sort: '-created',
        );
    // print(records);
    return records;
  ...
  static  postAsistenciaPk(TCheckPointsModel data) async {
    final record =
        await pb.collection('ar_chpr_meta').create(body: data.toJson());
  ...
  static  putAsitneciaPk({String? id, TCheckPointsModel? data}) async {
    final record =
        await pb.collection('ar_chpr_meta').update(id!, body: data!.toJson());
    return record;
  }

  static Future  deleteAsistentciaPk(String id) async {
    final record = await pb.collection('ar_chpr_meta').delete(id);
    return record;
  }
