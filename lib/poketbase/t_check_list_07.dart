...
class TChecklist_07 {
  ...
  static getAsitenciaPk() async {
    final records = await pb.collection('ar_chl_7_entrega_ropa_finisher').getFullList(
          sort: '-created',
        );
    // print(records);
    return records;
  ...
  static  postAsistenciaPk(TChekListmodel01 data) async {
    final record =
        await pb.collection('ar_chl_7_entrega_ropa_finisher').create(body: data.toJson());
    return record;
  }

  static  putAsitneciaPk({String? id, TChekListmodel01? data}) async {
    final record =
        await pb.collection('ar_chl_7_entrega_ropa_finisher').update(id!, body: data!.toJson());
    return record;
  }

  static Future  deleteAsistentciaPk(String id) async {
    final record = await pb.collection('ar_chl_7_entrega_ropa_finisher').delete(id);
    return record;
  }
