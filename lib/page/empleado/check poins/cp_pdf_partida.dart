...
class PDfChekPoint extends StatefulWidget {
  const PDfChekPoint(
      {super.key,
      ...
      this.screenshot});
  final List<TCheckPointsModel> toBeOmited; //sin accion
  final List<TCheckPointsModel> toBeUpdate;
  final List<TCheckPointsModel> toBeCreated;
  final List<TCheckPointsModel> toBeDeleted;
  final List<TCheckPointsModel> tolocalExist; //Solo existe localmente
  final String titlePDf;
  final String selectField;
  final Uint8List? screenshot;
  @override
  State<PDfChekPoint> createState() => _PDfChekPointState();
}

class _PDfChekPointState extends State<PDfChekPoint> {
  bool isSaving = false;

  @override
  Widget build(BuildContext context) {
    final dbProviderSQL = Provider.of<DBChPProvider00>(context);
...
pw.Column generateGridView(List<TCheckPointsModel> listData) {
  listData.sort((a, b) => (a.dorsal).compareTo(b.dorsal));
  if (listData.isEmpty) {
    return pw.Column(
      children: [
